...
public:
  ...
  int get_file(const std::string &name, bool is_dir, DWORD access_mode,
               DWORD shared_mode, std::unique_ptr<impl_file_handle> &out);
...
struct impl_chain_link {
  ...
  impl_chain_link() : prev_link_(nullptr) {
    memset(&call_ctx_, 0, sizeof(call_ctx_));
  }
};
...
public:
  ...
  int do_open_file(LPCWSTR FileName, DWORD share_mode, DWORD Flags,
                   PDOKAN_FILE_INFO DokanFileInfo);

  int do_create_file(LPCWSTR FileName, DWORD Disposition, DWORD share_mode,
                     DWORD Flags, PDOKAN_FILE_INFO DokanFileInfo);
  ...
  struct walk_data {
    ...
    std::vector<std::string> getdir_data; // Used only in
  ...
  int read_file(LPCWSTR file_name, LPVOID buffer, DWORD num_bytes_to_read,
                LPDWORD read_bytes, LONGLONG offset,
                PDOKAN_FILE_INFO dokan_file_info);

  int write_file(LPCWSTR file_name, LPCVOID buffer, DWORD num_bytes_to_write,
                 LPDWORD num_bytes_written, LONGLONG offset,
                 PDOKAN_FILE_INFO dokan_file_info);

  int flush_file_buffers(LPCWSTR file_name, PDOKAN_FILE_INFO dokan_file_info);
  ...
  int move_file(LPCWSTR file_name, LPCWSTR new_file_name, BOOL replace_existing,
                PDOKAN_FILE_INFO dokan_file_info);
...
public:
  impl_file_lock(impl_file_locks *_locks, const std::string &name)
      : locks(_locks), name_(name), first(NULL) {
...
public:
  ...
  int check_lock(long long start, long long len) {
    return file_lock->lock_file(this, start, len, false);
  }
  int lock(long long start, long long len) {
    return file_lock->lock_file(this, start, len);
  }
  int unlock(long long start, long long len) {
    return file_lock->unlock_file(this, start, len);
  }
};
