...
bool ready = false;
bool has_fillbuffer =
    false; // supports the FillBuffer API (platform is version 1.2 or higher)
cl::Context context;
cl::Device device;
...
#endif
    return queue.enqueueCopyBuffer(zero_buffer, buf, 0, 0, block::size, nullptr,
                                   nullptr);
}
...
static bool init_opencl() {
  ...
  for (auto &platform : platforms) {
    ...
    if (!has_fillbuffer) {
      ...
      int r;
      zero_buffer = cl::Buffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                               block::size, zero_data, &r);
      if (r != CL_SUCCESS)
        return false;
    }
...
int pool_available() { return pool.size(); }
...
block::~block() { pool.push_back(buffer); }
...
void block::write(off_t offset, size_t size, const void *data, bool async) {
  ...
  if (async) {
    event.setCallback(CL_COMPLETE, async_write_dealloc,
...
void block::sync() { last_write.wait(); }
}
}
