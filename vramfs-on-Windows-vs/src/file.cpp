...
namespace vram {
    namespace entry {
        ...
        int file_t::read(FUSE_OFF_T off, size_t size, char* data, std::mutex& wait_mutex) {
            if ((size_t) off >= _size) return 0;
			size = _size - off < size ? _size - off : size;

            // Walk over blocks in read region
			FUSE_OFF_T end_pos = off + size;
            size_t total_read = size;

            while (off < end_pos) {
                // Find block corresponding to current offset
				FUSE_OFF_T block_start = (off / memory::block::size) * memory::block::size;
				FUSE_OFF_T block_off = off - block_start;
                size_t read_size = memory::block::size - block_off<size? memory::block::size - block_off: size;
        ...
        int file_t::write(FUSE_OFF_T off, size_t size, const char* data, bool async) {
            ...
            while (off < end_pos) {
				...
				FUSE_OFF_T block_start = (off / memory::block::size) * memory::block::size;
				FUSE_OFF_T block_off = off - block_start;
                size_t write_size = memory::block::size - block_off< size? memory::block::size - block_off:size;
