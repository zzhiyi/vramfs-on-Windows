...
static int vram_statfs(const char*, struct statvfs* vfs) {
    ...
    vfs->f_ffree = std::numeric_limits<fsfilcnt64_t>::max();
...
static int vram_readlink(const char* path, char* buf, size_t size) {
    ...
    strncpy_s(buf, size,symlink->target.c_str(), size);
...
static int vram_readdir(const char* path, void* buf, fuse_fill_dir_t filler, FUSE_OFF_T, fuse_file_info*) {
    lock_guard<mutex> local_lock(fsmutex);
...
static int vram_truncate(const char* path, FUSE_OFF_T size) {
    lock_guard<mutex> local_lock(fsmutex);
